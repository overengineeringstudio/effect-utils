...
(
  ...
  grep -qxF "PNPM_CONFIG_STORE_DIR=$workspace/.devenv/pnpm-store" "$tmpdir/pnpm.log"
  grep -qxF "npm_config_store_dir=$workspace/.devenv/pnpm-store" "$tmpdir/pnpm.log"
  test -L "$workspace/.devenv/pnpm-store/v11/files"
  test "$(readlink "$workspace/.devenv/pnpm-store/v11/files")" = "$tmpdir/home/.local/share/pnpm/shared-files/v11"
)
...
(
  ...
  grep -qxF "PNPM_CONFIG_STORE_DIR=$workspace/.devenv/pnpm-store/nested" "$tmpdir/pnpm.log"
