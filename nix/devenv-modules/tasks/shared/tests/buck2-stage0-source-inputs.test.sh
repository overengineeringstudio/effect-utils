...
jq -e '
  (keys | sort) == ["archive-tool", "closure-tool", "package-evidence", "product", "typescript-product"]
' <<<"$inputs" >/dev/null || {
...
for tool in archive-tool closure-tool package-evidence product typescript-product; do
  ...
  for sibling in archive-tool closure-tool package-evidence product typescript-product; do
