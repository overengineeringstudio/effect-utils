...
while IFS= read -r candidate; do
  case "$candidate" in
    *agent-bin*) ;;
    *) real_nix="$(readlink -f "$candidate")"; break ;;
  esac
done < <(type -aP nix)
[[ -n "$real_nix" ]] || {
  echo "buck2-release-products-test: no unwrapped nix executable found" >&2
  exit 1
}
printf '{}\n' >"$live/outputs.json"
...
# runs are testable.
declare -A live_tag=() live_asset=() live_module=() live_entry=()
live_add_product() {
  ...
  live_entry["$name"]="$live/entry-$name.json"
}
...
live_add_package() {
  ...
  live_asset["$name"]="$asset"
  live_module["$name"]="$module"
  live_entry["$name"]="$live/entry-$transport_slug.json"
}
...
live_run() {
  ...
  local status=0
  local -a publisher_args=(--inventory "$inventory")
...
jq -e '.products[0].producerCommit == "0000000000000000000000000000000000000002"' \
