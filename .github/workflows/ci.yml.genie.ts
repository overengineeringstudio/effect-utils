  ...
  'devenv-perf': {
    ...devenvPerfJob({
      ...
      probes: [
        ...
        {
          id: 'debug_head_slowdown',
          label: 'Debug head-only slowdown',
          group: 'debug scenarios',
          path: ['debug', 'wall-clock', 'slowdown'],
          description:
            'Debug-only probe that sleeps on the PR head when the marker file exists. This should render as a paired wall-clock regression.',
          dimensions: {
            workload: 'debug-head-slowdown',
            debugScenario: 'regression',
          },
          warmupRepetitions: 0,
          repetitions: 3,
          gate: {
          ...
          command: [
            'bash',
            '-lc',
            'if [ -f .ci-measurement-debug-slow ]; then sleep 2; fi; "$DEVENV_BIN" processes --help >/dev/null',
          ],
        },
      ],
