  ...
  'devenv-perf': {
    ...devenvPerfJob({
      ...
      probes: [
        ...
        {
          id: 'debug_head_speedup',
          label: 'Debug head-only speedup',
          group: 'debug scenarios',
          path: ['debug', 'wall-clock', 'speedup'],
          description:
            'Debug-only probe that sleeps only in the paired base worktree because the marker file exists on the PR head. This should render as a paired wall-clock improvement.',
          dimensions: {
            workload: 'debug-head-speedup',
            debugScenario: 'improvement',
          },
          warmupRepetitions: 0,
          repetitions: 3,
          gate: {
            ...
            failAbs: 1,
            noiseFloor: 0.1,
            statisticalToleranceRatio: 0.05,
            statisticalToleranceAbs: 0.1,
          },
          command: [
            'bash',
            '-lc',
            'if [ ! -f .ci-measurement-debug-fast ]; then sleep 2; fi; "$DEVENV_BIN" processes --help >/dev/null',
          ],
        },
      ],
