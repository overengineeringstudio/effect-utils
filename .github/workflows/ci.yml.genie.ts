      ...
      },
      {
        ...
        run: withCiSourceRoot(
          [
            ...
            `tracked_product=$(git ls-files -- 'nix/buck2-products/**' | grep -Ev '^nix/buck2-products/(cache\\.nix|cache-targets\\.json|cache-targets\\.json\\.genie\\.ts|consumer-root\\.nix|default\\.nix|from-source\\.nix|manifest\\.json|publish\\.sh|source-recipes\\.nix|targets\\.json|targets\\.json\\.genie\\.ts)$' || true)`,
  ...
  'publish-products': {
    ...
    steps: [
      checkoutStep(),
      {
        ...
        name: 'Check publication scope',
        run: [
          'set -euo pipefail',
          'publish=true',
          'if [ "${GITHUB_EVENT_NAME:-}" = push ]; then',
          '  git fetch --no-tags --deepen=1 origin main',
          '  publish=false',
          '  while IFS= read -r changed; do',
          '    if [ "$changed" != nix/buck2-products/manifest.json ]; then',
          '      publish=true',
          '      break',
          '    fi',
      ...
      },
      {
        ...
        run: withCiSourceRoot(
          [
            ...
            'done',
            'nix build --no-link "${product_refs[@]}"',
