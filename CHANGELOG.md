...
- **@overeng/notion-react**: Retime the chaptered manual demo toward a one-minute final cut, switch the overlay progress dots to a segmented bar with per-chapter sync stats, and improve the lower-pane sync summary with colorized terminal output
- **@overeng/notion-react**: Replace the manual demo's frame-by-frame `screencapture` loop with a native ScreenCaptureKit region recorder, add nonce-based sync completion detection in tmux, and fail fast when the expected Ghostty/Chrome demo windows are not the ones currently on screen
...
- **@overeng/notion-react**: Replace the public `Lists and To-dos` demo's broken fake nested bullets with a supported toggle-based hierarchy fallback that reads cleanly on the published mobile page
