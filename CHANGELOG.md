...
### Changed
- **Buck2 observability VRS:** Specify pipeline-run trace identity, sealed
  record VCS fields, queued ingest/reconciliation, and PR trace access with
  an index-backed resolver and versioned agent JSON contract. This is a
  design/documentation change; service and CLI behavior are not yet shipped.
...
- **Genie**: Stage `#mr` members with the compiled import graph and give every
  ...
  cache from resolving an unchanged module through an earlier member pin. The
  compiled-mode serialization from #1359 stays as a conservative mitigation
  around the compiled Bun lifecycle; that cache behavior, not concurrency, was
  the root cause of the stale member pin.
...
- **@overeng/megarepo**: Make commit-worktree remnant recovery fail closed:
  `mr apply` removes only proven-empty, unregistered real directories and
  preserves every symlink, file, non-empty directory, registered path, or
  ambiguous target for manual inspection.
...
- **Buck2 (inert)**: added sandbox-free package, editor-view, and product
  ...
  Changed below.
- **Buck2 cache posture**: accepted decision 0033, with the public-versus-private
  CI trust matrix, candidate evidence, and isolated public cache tier.
