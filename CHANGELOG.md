...
- **Notion docs**: Align VRS specs with current Notion package implementation reality, including NotionMD V2 sidecars, datasource-sync domain/gateway boundaries, CLI command/output contracts, Notion React page ops/cache behavior, and open architecture questions for shared Notion domain/transport reuse.
...
- **@overeng/notion-cli**: Use the shared CLI build identity contract for `notion --version`.
- **@overeng/notion-cli**: Recompose `notion md` from the reusable `@overeng/notion-md` Effect command tree, keep `db`/`schema` native to the umbrella Effect CLI, and remove the legacy TypeScript-level CLI reinvocation path for markdown/sqlite dispatch.
...
- **@overeng/notion-datasource-sync**: Render sync progress for CLI sync operations through the shared `@overeng/tui-react` terminal UI, keeping machine-readable JSON results on stdout and progress on stderr.
- **@overeng/notion-datasource-sync**: Keep incremental query checkpoint identity stable across moving high-watermark windows, treat incomplete paginated page-property checkpoints as diagnostics instead of root-wide watch blockers, and preserve per-property write guards for incomplete relation-style surfaces.
- **@overeng/notion-datasource-sync**: Advance clean property base hashes after remote-only observations so later SQLite `rows` edits do not false-conflict, while keeping unresolved local property outbox entries pinned to their original base.
- **@overeng/notion-datasource-sync**: Add a typed bidirectional safety scenario matrix and VRS E2E plan covering false conflicts, same-property races, disjoint merges, lifecycle races, incremental checkpoint boundaries, relation pagination failures, ambiguous write retries, conflict-resolution lifecycle, rebuild/replay safety, local-first watch latency, and inline hydration correctness.
- **@overeng/notion-datasource-sync**: Add shared live Notion gateway request pacing, propagate 429 `Retry-After` metadata through gateway errors, persist outbox retry-after delays, and prevent retryable writes from being reclaimed before their retry time.
...
- **@overeng/notion-datasource-sync**: Enforce observed Notion `select` and `status` option sets in the public SQLite `rows` surface before queuing local CDC, and preserve schema property config when seeding CLI schema observations for tests and demos.
...
- **@overeng/notion-datasource-sync**: Add live scratch-row E2E coverage proving property-only SQLite edits stay body-scoped off, `.nmd` body edits settle cleanly, follow-up syncs no-op, and disposable fixtures clean up through the live ledger.
...
- **@overeng/notion-datasource-sync**: Wire `watch` to drain direct public SQLite `rows` CDC into planner intents, keep `status` non-clean while replica changes are pending, settle applied row creates back into `rows`, and skip local body-file scanning for data-only `--no-materialize-bodies` sync/watch cycles.
- **@overeng/notion-datasource-sync**: Treat SQLite rows and `.nmd` page bodies as one default sync experience while keeping body conflicts surface-scoped, so unchanged materialized bodies are not mistaken for local edits after remote body drift or unrelated property sync.
- **@overeng/notion-datasource-sync**: Preserve real body projections during body-suppressed sync and settle verified body pushes into the body projection and local `.nmd` clean base.
- **@overeng/notion-datasource-sync**: Add cleanup-ledger resume helpers, scenario metadata, and credential-free E2E coverage for live readiness lanes.
- **@overeng/notion-datasource-sync**: Collapse long-running reconciliation into `sync --watch`, add a durable webhook signal inbox, local Notion webhook receiver, Tailscale/manual relay provider boundary, signal-driven daemon wakeups, and docs/VRS/tests for webhook wakeups as non-authoritative dirty hints.
- **@overeng/notion-datasource-sync**: Promote the durable online demo page to an explicit live-demo manifest, add a read-only credentialed verifier that checks the current Notion page/data-source/schema/cardinality contract and syncs the fast demo sources into generated SQLite replicas, wire that verifier into the Notion integration task, and remove the misleading committed synthetic SQLite demo file.
- **@overeng/notion-datasource-sync docs**: Align live verification docs and scenario metadata with the canonical synthetic workspace contract: provisioner-owned stable IDs, durable read-only fixtures, scratch nursery write allowlists, marker-scoped ledger publishing, public-repo leak guard, and distinct #715/#717 live lanes.
- **@overeng/notion-datasource-sync**: Add credential-free manifest schema and guard tests for the public synthetic demo fixture, including an explicit provisioner lane contract and durable read-only 500+ row fixture requirement.
- **@overeng/notion-datasource-sync**: Let live demo verification resolve provisioned public synthetic data-source IDs by stable database titles and add a first-class `demo:provision` repair command.
- **@overeng/notion-datasource-sync**: Cover real NotionMD body materialization in the default live demo verifier by using the public synthetic projects fixture when no explicit existing body ref is configured.
- **@overeng/notion-datasource-sync**: Widen the explicit full live demo verifier timeout to match the 500-row local replica runtime envelope on shared development infrastructure.
- **@overeng/notion-datasource-sync**: Surface live Notion request and rate-limit state in CLI sync progress, backed by sanitized Notion HTTP telemetry events and OTel quota-routing span attributes.
- **@overeng/notion-datasource-sync**: Require independent read-after-write body observation before settling body pushes, and assert live scratch-row follow-up syncs do not mutate remote pages.
- **@overeng/notion-datasource-sync**: Strengthen the live scratch-row bidi proof with bounded `sync --watch` mutation settlement and a fixture-owned control row that must remain unchanged across property and body sync phases.
- **@overeng/notion-datasource-sync**: Add replica-derived `notion db export` support for rows, schema, sync status, pending changes, and conflicts in NDJSON or JSON, with optional pull-only `--from-notion` refresh and `--require-clean` enforcement.
- **@overeng/notion-datasource-sync**: Expose explicit public `sync_status.state` buckets for clean, pending, conflicted, unsupported, degraded, and incomplete SQLite replica states, including outbox, guard, capability, and hydration counts.
- **@overeng/notion-datasource-sync docs**: Collapse public docs, VRS CLI requirements, package metadata, and Nix exposure onto the single `notion db` surface, leaving replica/SQLite as domain terms and removing the standalone datasource-sync binary publication.
- **@overeng/notion-cli**: Remove the remaining unpromoted `migrate`/`repair` `notion db` leaves, rename the internal Node runtime helper away from the retired standalone binary name, and align docs/tests with the clean public command surface.
- **@overeng/notion-datasource-sync docs**: Add a human-facing VRS intuition companion explaining the local SQLite replica mental model, guarded sync flow, and fail-closed safety posture.
- **@overeng/notion-cli docs**: Add package-level VRS requirements, spec, and glossary for the umbrella CLI surface and Node-backed `notion db` runtime boundary.
...
- **@overeng/notion-datasource-sync**: Add the initial standalone Notion datasource sync package with VRS docs, Effect/Schema contracts, SQLite event/outbox/projection core, pure planner/guard/conflict logic, fake and live Notion gateways, real filesystem workspace materialization, NotionMD-backed `.nmd` body observation/materialization/push boundaries, one-shot and watch CLI surfaces, user conflict/delete/restore commands, OpenTelemetry instrumentation, and broad unit/integration/E2E coverage for pagination, high-watermarks, schema safety, body safety, conflicts, tombstones, daemon restart/cancellation/backpressure, live fixture ledgers, and intentional fail-closed unsupported surfaces.
- **@overeng/notion-datasource-sync**: Add the initial standalone Notion datasource sync package with VRS and developer-facing docs, Effect/Schema contracts, SQLite event/outbox/projection core, pure planner/guard/conflict logic, fake and live Notion gateways, real filesystem workspace materialization, NotionMD-backed `.nmd` body observation/materialization/push boundaries, one-shot and watch CLI surfaces, user conflict/delete/restore commands, OpenTelemetry instrumentation, an automated live Notion demo spanning multiple realistic data-source schemas including a 500-row high-cardinality surface, and broad unit/integration/E2E coverage for pagination, high-watermarks, schema safety, body safety, conflicts, tombstones, daemon restart/cancellation/backpressure, live fixture ledgers, and intentional fail-closed unsupported surfaces.
