...
- **nix packages**: Route Nix-built `notion sqlite` through a Node 24 datasource-sync runtime and expose `.#notion-datasource-sync` for direct SQLite replica workflows.
- **devenv**: Add the packaged `notion` CLI to the normal shell so `notion sqlite ...` uses the Node-backed runtime from `$PATH`.
- **@overeng/notion-cli**: Use the shared CLI build identity contract for `notion --version` and the Node-backed `notion sqlite --version` runtime.
- **@overeng/notion-datasource-sync**: Add a first-class `notion sqlite --help` surface for the packaged Node-backed markdown/SQLite runtime path and document unsupported Bun/source execution.
...
- **@overeng/notion-cli**: Compose `notion sqlite` into root help/completions from an import-safe datasource-sync Effect command tree while keeping packaged Nix/devenv execution on the Node-backed `node:sqlite` runtime.
