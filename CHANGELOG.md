...
- **CI / Nix packages**: Refresh stale pnpm fixed-output hashes for `oxc-config`, `genie`, and `notion-cli`; format PR-touched files and keep oxlint fatal for error-level diagnostics while the existing warning backlog is tracked separately.
