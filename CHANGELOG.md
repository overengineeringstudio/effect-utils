...
- **@overeng/tui-react**: Handle Ctrl-C for `run(App, handler)` at the TUI library layer. Apps whose action schema includes `Interrupted` now get scoped raw terminal input in progressive visual mode, interrupt the handler fiber through Effect, emit a `tui/ctrl-c/interrupt-handler` OTel span with `tui.interrupt.latency_ms`, dispatch `Interrupted` during normal `TuiApp` finalization, restore raw mode, and suppress noisy interrupt-only error output in `runTuiMain`.
