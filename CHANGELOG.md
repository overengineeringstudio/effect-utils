...
- **@overeng/notion-datasource-sync**: Extend the public SQLite CDC surface with typed body, metadata, schema, and conflict-resolution tables; promote row restore, body pushes, metadata title updates, and conservative schema operations into the guarded planner/outbox path; and keep row create, data-source descriptions, files, views, and unsafe conflict resolutions fail-closed.
