...
- **@overeng/notion-md, @overeng/tui-react**: Unified `/sk-cli-design` output for the notion-md CLI. Every write/inspect command (`edit`, `put`, `sync` incl. `--watch`, `status`, `plan`) now renders through the tui-react OutputMode seam — one tagged state schema per command driving the human view, the static `log`/`ci` modes, and `--output json`/`ndjson` from a single source — replacing the hardcoded `logJson` (which printed pretty JSON to stdout by default) and the hand-rolled `process.stderr.write` progress lines. **`--output` defaults to `auto`** (human view in a TTY, JSON when piped), a deliberate breaking change to the previous JSON-by-default stdout; pass `--output json` for the machine payload. Progress for the write commands flows from the unchanged engine `ProgressReporter` Tag through a `tui.dispatch` bridge Layer (so the engine stays render-agnostic — R45 byte-identical with the reporter absent/present/hostile), shown as a staged `TaskList`; drift surfaces as a `WARNING` badge with an actionable `fix:`, a blocking failure as a problems-first `CRITICAL`, and `sync --watch` as a live status line whose machine NDJSON stream stays byte-identical. A new shared `/sk-cli-design` kit lives in `@overeng/tui-react` (`cli-output/`: severity badges, problem-items with `fix:`/`skip:`, status glyphs, summary line, stage-list, the `CommandOutput` assembly), promoted from notion-md's stories so the vocabulary is real and reusable (notion-cli/megarepo can converge on it). `cat` is unchanged — a pure stdout Markdown pipe (excluded from the seam). Per the new VRS decision 0021 (avoid duplicating the official `ntn` CLI), the differentiated guarded write surface is kept because `ntn pages edit` is unguarded and R38-blind (silent data loss, #785) where ours refuses.
