...
fn read_manifest(path: &std::path::Path) -> ToolResult<Vec<Package>> {
    ...
    if manifest.schema_version != 1 {
        return Err(ToolError::new(
            ...
            format!(
                "unsupported closure manifest schemaVersion: {}",
    ...
    for (index, package) in manifest.packages.iter().enumerate() {
        if package.id.is_empty() {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_ID",
                format!("package entry {index} id must be a non-empty string"),
            ));
        }
        if prior_id.is_some_and(|prior| package.id.as_str() <= prior) {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_ORDER",
                "closure manifest package ids must be unique and strictly increasing",
            ));
        }
        ...
        if package.projection_path == RESERVED_METADATA_PATH
            || package
                .projection_path
                .starts_with(&format!("{RESERVED_METADATA_PATH}/"))
        {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_RESERVED",
                format!(
                    "projectionPath collides with reserved metadata path: {}",
        ...
        if !seen.insert(package.projection_path.clone()) {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_DUPLICATE",
                format!("duplicate projectionPath: {}", package.projection_path),
            ));
        }
        if let Some(prior) = paths.iter().find(|prior| {
            is_ancestor(prior, &package.projection_path)
                || is_ancestor(&package.projection_path, prior)
        }) {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_COLLISION",
                format!(
                    "projectionPath file/ancestor collision: {prior} and {}",
                    package.projection_path
                ),
            ));
        }
...
fn stage(args: StageArgs) -> ToolResult<()> {
    let widths = [
        args.package_ids.len(),
        args.projection_paths.len(),
        args.sha256s.len(),
        args.artifacts.len(),
    ];
    if widths.iter().any(|width| *width != widths[0]) {
        return Err(ToolError::new(
            "BUCK2_CLOSURE_EDGES",
            "each package requires id, projection path, sha256, and artifact",
        ));
    }
    let declared = args
        .package_ids
        .into_iter()
        .zip(args.projection_paths)
        .zip(args.sha256s)
        .zip(args.artifacts)
    ...
    for (package, artifact) in &declared {
        ...
        if actual != package.sha256 {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_DIGEST",
                format!(
                    "package digest mismatch for {}: expected {}, got {actual}",
                    package.id, package.sha256
                ),
            ));
        }
        ...
        if let Some(parent) = destination.parent() {
            fs::create_dir_all(parent)
                .map_err(|error| ToolError::new("BUCK2_CLOSURE_IO", error.to_string()))?;
        }
    ...
    fs::write(
        args.out.join(RESERVED_METADATA_PATH),
...
fn probe(args: ProbeArgs) -> ToolResult<()> {
    ...
    for package in manifest {
        ...
        if actual != package.sha256 {
            return Err(ToolError::new(
                "BUCK2_CLOSURE_DIGEST",
                format!("staged package digest mismatch for {}", package.id),
            ));
        }
    ...
    fs::write(args.out, pretty_json(&evidence)?)
        .map_err(|error| ToolError::new("BUCK2_CLOSURE_IO", error.to_string()))?;
    Ok(())
}
...
fn make_tree_read_only(root: &std::path::Path) -> ToolResult<()> {
    fn visit(path: &std::path::Path) -> ToolResult<()> {
        ...
        {
            let entry =
                entry.map_err(|error| ToolError::new("BUCK2_CLOSURE_IO", error.to_string()))?;
            let ty = entry
                .file_type()
                .map_err(|error| ToolError::new("BUCK2_CLOSURE_IO", error.to_string()))?;
            if ty.is_dir() {
...
mod tests {
    ...
    #[test]
    fn accepts_strict_shape() {
        let (_d, p) = manifest(
            serde_json::json!({"packages":[{"id":"a","projectionPath":"a","sha256":"0".repeat(64)}],"schemaVersion":1}),
        );
        assert_eq!(read_manifest(&p).unwrap()[0].id, "a");
    }
    #[test]
    fn rejects_excess_fields() {
        let (_d, p) =
            manifest(serde_json::json!({"packages":[],"schemaVersion":1,"unexpected":true}));
    ...
    fn rejects_noncanonical_order() {
        let (_d, p) = manifest(
            serde_json::json!({"packages":[{"id":"z","projectionPath":"z","sha256":"0".repeat(64)},{"id":"a","projectionPath":"a","sha256":"0".repeat(64)}],"schemaVersion":1}),
        );
        assert!(read_manifest(&p)
            .unwrap_err()
            .message
    ...
    fn rejects_path_traversal() {
        ...
        assert!(read_manifest(&p).unwrap_err().message.contains("traverse"));
    }
    #[test]
    fn rejects_reserved_manifest_path() {
        let (_d, p) = manifest(
            serde_json::json!({"packages":[{"id":"a","projectionPath":"closure-manifest.json","sha256":"0".repeat(64)}],"schemaVersion":1}),
        );
        assert!(read_manifest(&p).unwrap_err().message.contains("reserved"));
    }
    #[test]
    fn rejects_reserved_manifest_ancestor() {
        let (_d, p) = manifest(
            serde_json::json!({"packages":[{"id":"a","projectionPath":"closure-manifest.json/payload","sha256":"0".repeat(64)}],"schemaVersion":1}),
        );
        assert!(read_manifest(&p).unwrap_err().message.contains("reserved"));
    }
    #[test]
    fn rejects_file_ancestor_collision() {
        let (_d, p) = manifest(
            serde_json::json!({"packages":[{"id":"a","projectionPath":"x","sha256":"0".repeat(64)},{"id":"b","projectionPath":"x/y","sha256":"0".repeat(64)}],"schemaVersion":1}),
        );
        assert!(read_manifest(&p)
            .unwrap_err()
            .message
            .contains("file/ancestor collision"));
    }
}
