...
- Each repo keeps `.envrc` minimal: it sources `.envrc.generated.nix` and `.envrc.local`, and includes repo-specific setup inline.
- `.envrc.generated.nix` is generated by the temporary `megarepo-nix sync` helper and ensures the local Nix workspace exists under `.direnv/megarepo-nix/workspace`.
- `.envrc.local` remains developer overrides.
- The local workspace avoids `path:.` eval hashing huge ignored trees in the repo, which can otherwise dominate flake eval time.

## build path semantics

- Inside a megarepo, always build/eval from the local workspace path (`.direnv/megarepo-nix/workspace/<repo>`).
- There is no separate "dirty" mode inside the megarepo; the local workspace always reflects current working tree state.
- Outside the megarepo, use the repo root as normal (`nix build path:.#...` or `nix build .#...`).
