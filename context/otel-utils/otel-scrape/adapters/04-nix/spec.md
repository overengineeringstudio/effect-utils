...
| Telemetry constants        | **planned** `context/otel-utils/otel-scrape/telemetry-registry.json` — spans `nix.build`/`nix.substitute` (or `nix.activity`); attrs `otel_scrape.adapter.nix.*` (ADP-R06) → `src/telemetry_registry.gen.rs` (genie)                                                                                    |
