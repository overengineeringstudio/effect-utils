...
This document specifies _how_ the `oxlint` adapter works. It builds on its
...
| Telemetry constants        | `context/otel-utils/otel-scrape/telemetry-registry.json` (`oxlint_diagnostics`; adapter-event attrs `adapter_event_{severity,source_filename_hash,rule,line}`) → generated `src/telemetry_registry.gen.rs` (genie)                                   |
...
- **stdout ownership:** needs-render. `--format=json` _replaces_ oxlint's human
