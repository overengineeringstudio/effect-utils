...
This document specifies _how_ the `pnpm` adapter works. It builds on its
...
| Telemetry constants        | **planned** `context/otel-utils/otel-scrape/telemetry-registry.json` — spans `pnpm.resolve`/`pnpm.import`; attrs `otel_scrape.adapter.pnpm.*` (ADP-R06) → `src/telemetry_registry.gen.rs` (genie)                                                                                                                                                                           |
...
_documented named reporter_, not `--loglevel debug` scraping.

## Source (ADP-R01 phase lane)

- **Flag (declared):** `--reporter=ndjson` — bunyan-style NDJSON on stdout,
  events keyed by `name`. The flag is stable; the per-event _payload_ schema
