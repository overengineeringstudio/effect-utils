...
| Telemetry constants        | `context/otel-utils/otel-scrape/telemetry-registry.json` — metric `deadnix.findings` (mirrors `oxlint_diagnostics`; reuses the adapter-event attrs) → generated `src/telemetry_registry.gen.rs` (genie)                                                                                                 |
