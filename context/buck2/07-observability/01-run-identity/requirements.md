...
`BUCK_WRAPPER_UUID`, the per-invocation sidecar line, and span-id salting. It
refines BUCK.OBS-R03 and BUCK.OBS-R07 of the
[07-observability requirements](../requirements.md).

## Assumptions

- **BUCK.OBS.ID-A01 Direct invocation:** Buck is invoked directly
...
- **BUCK.OBS.ID-T01 One wrapper process per invocation:** the `otel-span`
...
- **BUCK.OBS.ID-R01 Command span per invocation (refines BUCK.OBS-R03):**
...
- **BUCK.OBS.ID-R03 Validate before export (refines BUCK.OBS-R07):** The W3C
  ...
  malformed export.
- **BUCK.OBS.ID-R04 Sidecar line:** Each invocation appends one line
...
- **BUCK.OBS.ID-R05 Salted span ids:** Exported OTLP span ids are salted
  deterministically per invocation (a function of log identity and Buck span
  id), making concurrent and repeated commands collision-free and re-pushes
  idempotent.
- **BUCK.OBS.ID-R06 Nested and repeated invocations:** Multiple Buck commands
