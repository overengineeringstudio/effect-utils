...
Repos that depend on effect-utils via megarepo need to run `mr apply` in CI before devenv is available. This creates a chicken-and-egg problem:
...
3. We can't run `mr apply` without `mr`
...
- name: Sync megarepo dependencies
  run: bunx @overeng/megarepo apply
  shell: bash
```
...
#### Option 4: Skip `mr apply` - Clone Manually
...
1. ✅ `mr apply` now allows cloning in fresh CI environments (fix in commit 693eb6b)
