...
The current `tui-core` implementation stops after the exact contextual input
plan. It does not fetch, unpack, normalize, or materialize package payloads and
therefore cannot mint authoritative package or task-closure identities. The
generated plan is review evidence only; admission requires package actions and
an isolated TypeScript consumer action connected through the real Buck graph.
