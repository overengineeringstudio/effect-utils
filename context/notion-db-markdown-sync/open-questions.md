...
## OQ3 — Transactional replica schema creation

`createReplicaSchema` installs replica schema objects and CDC triggers outside a
single transaction. Make schema and trigger creation atomic so there is no window
where one exists without the other.

## OQ4 — Declared traceability scenarios without executable tests
...
## OQ5 — Permanent delete after local archive
