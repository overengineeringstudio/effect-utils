...
| Adapter implementation | `packages/@overeng/otel-scrape/src/lib.rs` — `NODE_CPUPROFILE_ADAPTER`, `prepare_node_cpuprofile_dir()`, `ProfileLink`, `validate_cpuprofile_file()`; `stdout_mode` = `TeeLive`; CAS write via `content_address.rs` |
