...
- **R31 Root trace surfacing (terminal-only):** When `otel-scrape` mints the trace root (no inbound `traceparent`) and telemetry is active for the run, it surfaces the trace identity to the operator's terminal (stderr) so agents and humans can open or correlate the trace without first querying a backend. Surfacing is **terminal-only**: the terminal is not a sink (decision 0017), so the surfaced text never enters the summary or OTLP export, and it carries only the public-safe random trace id plus an operator-configured, backend-agnostic URL template — never argv, cwd, paths, or credentials. Surfacing respects passthrough fidelity (R03) and disabled-mode transparency (R04): pure passthrough (no summary, no export) emits nothing, and the wrapped command's stdout/stderr/exit are unaffected. A **resolvable URL** is surfaced only when the export is acknowledged (the collector returns a 2xx for the command span) — acknowledgement of receipt, not a guarantee of downstream ingestion; when export is unavailable, disabled, or fails but a local sink (summary) or attempted export still makes the id useful, the **bare trace id** is surfaced instead. The operator can disable surfacing; there is no override past the R04 gate (decision 0020).
