...
**Decision:** Implement the first OTLP slices as a small first-party OTLP/HTTP JSON exporter boundary. It is enabled only when an endpoint is configured, emits the wrapper command span first, adds adapter-derived span events and profile-link span events on the same command span, and treats exporter failures as degraded evidence that never changes child stdout, stderr, stdin, or exit status.
