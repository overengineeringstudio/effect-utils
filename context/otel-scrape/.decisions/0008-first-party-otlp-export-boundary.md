...
**Decision:** Implement the first OTLP slice as a small first-party OTLP/HTTP JSON exporter boundary. It is enabled only when an endpoint is configured, emits the wrapper command span first, and treats exporter failures as degraded evidence that never changes child stdout, stderr, stdin, or exit status. Adapter events and profile links use the same exporter boundary in follow-up slices.
