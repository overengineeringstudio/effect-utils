...
const plannedPropertyCommand = () => {
  const decision = planIntent({
    ...
    intent: propertyEditIntent({ desiredHash: expectedPatchHash() }),
  })
...
const schemaMigrationIntent = (
  ...
  const command = decode({
    schema: PatchDataSourceSchemaCommand,
    value: {
...
describe('notion datasource sync fake-service E2E harness', () => {
  ...
  it('composes fake gateway, body adapter, workspace, clock, and SQLite store for clean status', async () => {
    ...
    try {
      ...
      const localObservations = await collectStream(
        ports.workspace.scan(
          decode({ schema: AbsolutePath, value: '/tmp/notion-ds-sync-fixture' }),
        ),
      )
  ...
  it('keys remote observations by projection-affecting schema, lifecycle, and sidecar state', async () => {
    ...
    const schemaKey = (result: Awaited<ReturnType<typeof observeFakeRemote>>) =>
      result.events.find((event) => event._tag === 'DataSourceSchemaObserved')?.idempotencyKey
    ...
    expect(dataSourceKey(base)).toBe(dataSourceKey(reconfigured))
    expect(schemaKey(base)).not.toBe(schemaKey(reconfigured))
  ...
  it('rejects body adapter surface leaks and records no outbox settlement', async () => {
    ...
    const decision = planIntent({
      snapshot: buildPlannerSnapshot(),
      intent: bodyAdapterResultIntent(bodySafety({ adapterMutationSurfaces: ['body', 'schema'] })),
    })
    const storeFixture = makeStoreFixture({ mode: 'memory' })
