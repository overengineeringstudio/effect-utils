...
describe('notion datasource sync observability', () => {
  ...
  it('keeps CLI, one-shot, and daemon spans wired through the shared catalog', () => {
    expect(source('cli.ts')).toContain('Effect.fn(spanNames.cliCommand, {')
    expect(source('cli.ts')).toContain(
      'makeOtelCliLayer({ serviceName: serviceNameForArgv(argv) })',
    )
    expect(source('sync.ts')).toContain('Effect.fn(spanNames.syncPull)')
    expect(source('sync.ts')).toContain('Effect.fn(spanNames.syncPush)')
    expect(source('sync.ts')).toContain('Effect.fn(spanNames.syncOneShot)')
    expect(source('daemon.ts')).toContain('Effect.fn(spanNames.daemonPass, {')
    expect(source('daemon.ts')).toContain('Effect.fn(spanNames.daemonRun, {')
  })
})
