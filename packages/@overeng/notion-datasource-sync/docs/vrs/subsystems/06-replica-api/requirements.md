...
- **REPLICA-R12 Archive/restore round-trip:** A page archived via `_in_trash` 0→1 pushes to remote trash, and the trashed state is retained across reprojection (remote-trash reprojects `_in_trash = 1`) so the page stays a restorable row. A local restore (`_in_trash` 1→0) emits a restore intent and pushes a remote restore; restoring a page that has moved out of the tracked data source is guarded. A page permanently deleted in Notion (past the trash window) while locally archived fails closed on restore — a documented limitation. The planner-side restore lifecycle is governed by the canonical lifecycle requirements [[PLAN-R09]], [[PLAN-R12]], and [[PLAN-R13]] (first-class restore, moved-out classification, and permission fail-closed); this requirement adds the replica reprojection-retention invariant that makes local restore expressible. _(Records [decision 0022](../../../../../../../context/notion-sync-architecture/02-realizations/01-datasource-markdown-workspace/.decisions/0022-lifecycle-is-a-planner-concern.md) and [decision 0023](../../../../../../../context/notion-sync-architecture/02-realizations/01-datasource-markdown-workspace/.decisions/0023-archive-restore-retains-trashed-state.md).)_
