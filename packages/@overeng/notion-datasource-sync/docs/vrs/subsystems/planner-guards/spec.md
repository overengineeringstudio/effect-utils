...
| `PendingIntentShadowViolation`       | Remote observation would overwrite pending or settled local target state       | Reject projection mutation; raise `lifecycle` conflict (decision 0026) or require repair             | `ConflictDetected` (lifecycle) / `RepairObserved` |
