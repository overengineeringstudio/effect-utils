...
| `DeleteFromRowsArchiveIntent`        | `DELETE FROM rows WHERE ...` is observed via local CDC                         | Enqueue a remote archive (trash) intent, identical to `_in_trash = 1`; this reversible op is the maximum destructive effect reachable from SQL | `LocalIntentAccepted` archive intent           |
...
from a bare filesystem delete: it is a deliberate SQL intent and is mapped to a
reversible remote archive (trash), as specified by `DeleteFromRowsArchiveIntent`
above.
