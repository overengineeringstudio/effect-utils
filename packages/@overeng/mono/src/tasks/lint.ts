 ...
 *
 * Uses default config file (.oxlintrc.json).
 */
...
export const lintCheck = (config?: OxcConfig) =>
  runCommand({
    command: 'oxlint',
    args: ['--import-plugin', '--deny-warnings', ...(config?.extraLintArgs ?? [])],
...
export const lintFix = (config?: OxcConfig) =>
  runCommand({
    command: 'oxlint',
    args: ['--import-plugin', '--deny-warnings', ...(config?.extraLintArgs ?? []), '--fix'],
...
export const allLintFixes = (oxcConfig?: OxcConfig) =>
  Effect.all([formatFix(), lintFix(oxcConfig)], {
    concurrency: 'unbounded',
