...
import { Array, Console, Effect, Layer, Ref } from 'effect'
...
describe('Inline renderer', () => {
  it.live('does not duplicate task list between live updates and final summary', () =>
    withTestCtx(
      captureConsoleOutput(
        ...
        Effect.map(({ output }) => {
          // Count how many times each task appears in output
          const task1Count = output.filter((line) => line.includes('Fast task 1')).length
          const task2Count = output.filter((line) => line.includes('Fast task 2')).length
          ...
          const summaryLineIndex = output.findIndex((line) => line.includes('All') && line.includes('completed'))
  ...
  it.live('shows log lines for failed tasks', () =>
    withTestCtx(
      captureConsoleOutput(
        Effect.gen(function* () {
          const renderer = inlineRenderer()
          ...
          const tasks = [
            task('failing', 'Failing task', {
              cmd: 'sh',
              args: ['-c', 'echo "Line 1" && echo "Line 2" && echo "Line 3" && >&2 echo "ERROR" && exit 1'],
            }),
          ]
  ...
  it.live('truncates long log lines at 80 characters', () =>
    withTestCtx(
      captureConsoleOutput(
        Effect.gen(function* () {
          const renderer = inlineRenderer()
  ...
  it.live('does not show log lines for successful tasks', () =>
    withTestCtx(
      captureConsoleOutput(
        Effect.gen(function* () {
          const renderer = inlineRenderer()
        ...
        Effect.map(({ output }) => {
          ...
          const hasLogLines = output.some((line) => line.includes('│') && line.includes('completed'))
  ...
  it.live('shows summary message after task list', () =>
    withTestCtx(
      captureConsoleOutput(
        Effect.gen(function* () {
          const renderer = inlineRenderer()
