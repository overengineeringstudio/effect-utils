...
die)` (R13). The durable combinators thus have a CLEAN `E` (§6), and only the
...
Every durable combinator has a CLEAN `E` (no `RestateError`, #1, §5): an infra
failure is a defect classified at the boundary, so a handler with no declared
domain error keeps `E = never`. Explicit signatures:
...
- `Restate.retryable(errorSchema, { retryAfter? })` / a `RetryableError` as the
  ...
  `toTerminal` (#3, [decisions/0011](./decisions/0011-restate-schema-annotations.md)).
...
### 9.3 Self-reschedule and durable scheduling
...
- `onCycleError`: `skipToNext` (DEFAULT — swallow a failing cycle, keep cadence) or
  ...
  `RestateError` defect (§5); an interrupt is re-raised.
