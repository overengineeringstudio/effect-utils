...
`restate.{service,handler,object.key}` plus `restate.workflow.id` (the Workflow
key) and `restate.idempotency.key` (the original-invocation idempotency key)
(identity), and, on a failure, `restate.error.{tag,class}` (`class` ∈
`terminal`/`retryable`/`cancelled`, read from the boundary's `classifyOutcome`).
`workflow.id`/`idempotency.key` are identity values, never a redacted FIELD.
...
`query`/`queryRaw`) are Effects failing with `RestateError({ reason: 'AdminFailed' })`.
The raw HTTP is shared with the **Test Harness** via one bare client (`AdminApi.ts`).
_Avoid_: "ingress client" (that invokes handlers; admin OPERATES the deployment).
...
CALLER owns the SQL AND the row Schema (the binding does not own the `sys_*` shapes),
and the binding only threads them through + `Schema.decodeUnknown`s each row (a
mismatch → `AdminFailed`). `queryRaw` is the untyped escape hatch.

**Stuck delivery**:
An operationally-wedged invocation an operator surfaces via the admin `/query`: a
NON-terminal `sys_invocation` row (`backing-off` = retrying a retryable error;
`suspended` = a Service/Object parked on a durable wait; a long-`running` Workflow
`run`, which on 1.6.2 does NOT flip to `suspended`), ranked by `retry_count`, then
inspected and `cancel`/`kill`/`release`d. The Molty runbook
