...
Effect _idioms_ on top (Schema-based I/O + error boundary, Layers/Scopes for
...
- Effect already ships `@effect/workflow` + `@effect/cluster` for those who want
  Effect's _own_ durable engine. A second, lower-fidelity abstraction over
...
- "Fully Effect-idiomatic" is achieved through _how_ the surface is exposed
