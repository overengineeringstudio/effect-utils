...
OUTSIDE `Restate.run` and the journaled Clock/Random. Because `ctx.rand` is
disallowed inside `ctx.run`, an id generated inside a `Restate.run` closure uses
`crypto.randomUUID()` directly (a documented lint exemption strictly inside
`Restate.run`); an id generated OUTSIDE `run` uses the journaled `Random`.
...
- `unsafeCurrentTimeMillis`'s frozen-per-attempt base is a design choice to
  validate against a representative handler (a spec DQ).
...
frozen-per-attempt monotonic base for the sync `unsafeCurrentTime*` reads and the
`ctx.run` lint exemption._
