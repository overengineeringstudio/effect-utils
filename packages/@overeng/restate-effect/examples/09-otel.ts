 ...
 *
 * `RestateOtel.withOtel(endpointOptions)` composes the hook + bridge onto the core
 * `EndpointOptions`; pair it with `RestateOtel.layer` provided over the
 * application Layer. The otel packages live behind this subpath only, so the core
...
import { layer, type RestateError, serve } from '../src/mod.ts'
 ...
 * exporter; a `ConsoleSpanExporter` is fine to see the one-trace story locally. */
export const OtelLayer = RestateOtel.layer({
  resource: { serviceName: 'greeter', serviceVersion: '1.0.0' },
  exporter: new ConsoleSpanExporter(),
})

/* `withOtel` attaches the hook + inbound bridge to every served handler. */
