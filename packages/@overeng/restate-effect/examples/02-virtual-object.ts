...
export const CounterObj = RestateObject.contract({
  name: 'counter',
  def: {
    state: CounterState,
    handlers: {
...
export const CounterLive = RestateObject.implement<typeof CounterObj>({
  ...
  impl: {
    ...
    add: (amount) =>
      Effect.gen(function* () {
        ...
        yield* Counter.set({ key: 'count', value: next }) // requires StateWrite — only legal here
        return next
      }),
    /* Shared (read-only): a `Counter.set({ key: 'count', value: … })` here would be a COMPILE error
