 ...
 */
export const greetWithRecovery = callTyped({
  contract: Greeter,
  method: 'greet',
  input: { name: '' },
}).pipe(
...
export const addToCounter = objectCall({
  contract: CounterObj,
  key: 'counter-1',
  method: 'add',
  input: 3,
...
export const runApproval = Effect.gen(function* () {
  yield* workflowSubmit({ contract: ApprovalWf, key: 'wf-1', input: 'please review' })
  yield* workflowCall({ contract: ApprovalWf, key: 'wf-1', method: 'approve', input: undefined }) // a signal
  return yield* workflowAttach({ contract: ApprovalWf, key: 'wf-1' }) // awaits the run's typed success
}).pipe(Effect.provide(IngressLayer))
