...
1. **Silent surface drift.** A duplicated command tracks a moving upstream. If
   we forget *why* we duplicated it, we cannot tell an intentional divergence
...
2. **False safety.** `ntn`'s page-edit path is an **unguarded last-writer-wins**
   ...
   `link_preview`, `link_to_page`, and a body `child_page`) render to *plausible*
...
Writing the principle down means every overlap is a *recorded* decision, not an
...
| `notion schema`, `notion db sync/export`   | (none)                             | **KEEP** *(notion-cli)* | Schema codegen and guarded DB-row sync. No `ntn` equivalent. Owned by `@overeng/notion-cli`, listed for completeness.                                                                                                                              |
| `cat`                                      | `ntn pages get`                    | **DEFER / RECONSIDER** | Read-only; the only edge is base-hash emission (the optimistic-concurrency token the guarded `put` consumes) plus refuse-on-read for lossy pages. For a casual read, `ntn pages get` is *better* (it surfaces a truncation warning we don't). Tracked as an open question. |
| raw datasource query / `api` / `files` / `login` | `ntn datasources`/`api`/`files`/`login` | **DEFER (no duplicate)** | Already `ntn`'s; we do not re-implement them. The steady-state boundary: our tooling is body/page editing + multi-page sync *on top of* `ntn`, and reaches for raw `ntn` for everything else (queries, uploads, one-off API calls, auth).        |
...
- Decision [0004](./0004-umbrella-surfacing.md) positions `notion edit` as the
  ...
  `$EDITOR` natively, that *positioning* should lean on the guard/refuse-lossy
