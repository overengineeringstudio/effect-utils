...
hashed/compared, and the body pushed are the *same canonical bytes*.
...
- The canonical function lives in `@overeng/notion-effect-client`, beside the
  ...
  stays in `@overeng/notion-md` — it is sync *policy*, not the wire form.
...
lists, and was only *masked* (not caught) by `semanticEquivalent`'s
...
- **The vestigial client `markdownToBlocks` converter is deleted.** Under
  refuse-lossy / one-engine (decisions 0015/0016), push sends a raw Markdown
...
- **Body bytes change → body hashes change, once.** Any already-synced page with
  ...
  stays *masked* by `semanticEquivalent` — pre-existing, not changed by this
  consolidation.

## Deliberate non-changes

- **`normalizeMarkdownLineEndings` stays** as the line-level sub-step (on-disk /
  title-frame / hash-prep). It is a step *of* the canonical function, not a
...
- **`normalizeComparableMarkdown` / `normalizeLines` stay in `notion-core`.**
  ...
  *depends on* core, so the constraint is permanent. The per-line `trimEnd` used
