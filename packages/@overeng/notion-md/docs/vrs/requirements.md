...
- **R37 Single production reconcile engine:** The production implementation must have one frontmatter-dispatched reconcile engine used by `status`, one-shot `sync`, and `sync --watch`. Alternate push/pull/sync/status paths must not remain as production paths with different safety semantics.
