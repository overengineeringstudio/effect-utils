...
Vitest.describe('opencode adapter integration', () => {
  ...
  Vitest.it.effect('does not skip OpenCode rows that share the checkpoint timestamp', () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem
      const tempDir = yield* fs.makeTempDirectoryScoped()
      const databasePath = nodePath.join(tempDir, 'opencode.db')
      ...
      yield* Effect.sync(() => {
        ...
        try {
          ...
          db.prepare(
            `insert into session (id, project_id, slug, directory, title, version, time_created, time_updated)
             values (?, 'proj_1', ?, ?, ?, ?, ?, ?)`,
          ).run('ses_2', 'steady-meadow', '/tmp/repo', 'Watermark session', '1.2.15', 1000, 2000)
          db.prepare(
            `insert into message (id, session_id, time_created, time_updated, data)
             values (?, ?, ?, ?, ?)`,
      ...
      const artifact = yield* expectSingleArtifact(adapter)
      const first = yield* adapter.ingestArtifact({ artifact, checkpoint: undefined })
      expect(first.records.some((record) => record._tag === 'OpenCodeMessage')).toBe(true)
