  ...
  Schema.transformOrFail(Schema.String, schema, {
    ...
    decode: (text) =>
      Effect.gen(function* () {
        ...
        return yield* Schema.decodeUnknown(schema)(normalized).pipe(Effect.mapError((e) => e.issue))
      }),
    encode: (value) =>
      Effect.gen(function* () {
        const raw = yield* Schema.encodeUnknown(schema)(value).pipe(Effect.mapError((e) => e.issue))
