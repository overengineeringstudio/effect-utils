...
const allPages = yield* NotionDatabases.queryStream({ databaseId: '...' })
...
const newPage = yield* NotionPages.create({
...
const allChildren = yield* NotionBlocks.retrieveChildrenStream({ blockId: '...' })
  .pipe(Stream.runCollect)
...
const blocks = yield* NotionBlocks.retrieveAllNested({ blockId: pageId })
  .pipe(Stream.runForEach((item) =>
...
const results = yield* NotionSearch.search({
  query: 'meeting notes',
  filter: { property: 'object', value: 'page' },
})

// Stream all search results
const allResults = yield* NotionSearch.searchStream({ query: 'project' })
  .pipe(Stream.runCollect)
```
...
const result = yield* NotionPages.retrieve({ pageId: '...' }).pipe(
