...
const bundleProduct = (prefix: string): { readonly bytes: string; readonly descriptor: string } => {
  ...
  const result = Bun.spawnSync({
    cmd: [
      process.execPath,
      join(dirname(fileURLToPath(import.meta.url)), 'package-command-runner.ts'),
      'bundle',
      process.execPath,
      packageTree,
      'src/cli.ts',
...
describe('a bundled portable product', () => {
  ...
  it('emits the exact portable module-v2 descriptor and no host provenance', () => {
...
describe('package command runner', () => {
  ...
  it('normalizes and deduplicates declared read roots', () => {
    ...
    ).toStrictEqual(['/a-dependency', '/z-dependency'])
  })
