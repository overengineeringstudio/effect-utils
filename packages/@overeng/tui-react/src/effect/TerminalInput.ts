    ...
    Effect.sync(() => {
      ...
      dataHandler = (data: Buffer) => {
        ...
        for (const event of events) {
          ...
          void Effect.runForkWith(runtime)(PubSub.publish(pubsub, event))
        }
      }
      ...
      if (handleResize === true && output.isTTY === true) {
        resizeHandler = () => {
          ...
          void Effect.runForkWith(runtime)(PubSub.publish(pubsub, resizeEvent({ cols, rows })))
        }
...
export const createTerminalResize = Effect.fn('TerminalResize.create')(function* (
  ...
  if (output.isTTY === true) {
    ...
    const resizeHandler = () => {
      ...
      void Effect.runForkWith(runtime)(PubSub.publish(pubsub, dims))
    }
