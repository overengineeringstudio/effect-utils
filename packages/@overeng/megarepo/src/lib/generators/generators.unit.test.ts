 ...
 *
 * Tests the pure content-generating functions for envrc, vscode, and schema generators.
 */

import { describe, expect, it } from 'vitest'

import { EffectPath, type MegarepoConfig } from '../config.ts'
import { generateEnvrcContent } from './envrc.ts'
...
// =============================================================================
// Envrc Generator Tests
// =============================================================================

describe('envrc generator', () => {
  describe('generateEnvrcContent', () => {
    it('should generate envrc content with member names', () => {
      ...
      expect(content).toContain('MEGAREPO_ROOT="/test/megarepo/"')
      expect(content).toContain('MEGAREPO_MEMBERS="effect,schema"')
      expect(content).toContain('# Generated by megarepo')
      expect(content).toContain('# Regenerate with: mr generate')
    })
    ...
    it('should use absolute path for MEGAREPO_ROOT', () => {
      const customRoot = EffectPath.unsafe.absoluteDir('/home/user/workspace/')
      const content = generateEnvrcContent({
        megarepoRoot: customRoot,
        config: createTestConfig({}),
      })

      expect(content).toContain('MEGAREPO_ROOT="/home/user/workspace/"')
    })
  })
})
...
describe('vscode generator', () => {
  describe('generateVscodeContent', () => {
    it('should generate vscode workspace with all members', () => {
      ...
      expect(workspace.folders).toContainEqual({ path: 'lib1', name: 'lib1' })
      expect(workspace.folders).toContainEqual({ path: 'lib2', name: 'lib2' })
    })
    ...
    it('should exclude members from options.exclude', () => {
      ...
      expect(workspace.folders).not.toContainEqual(expect.objectContaining({ path: 'large-repo' }))
    })
    ...
    it('should exclude members from config.generators.vscode.exclude', () => {
      ...
      expect(workspace.folders).not.toContainEqual(expect.objectContaining({ path: 'excluded' }))
    })
    ...
    it('should prefer options.exclude over config.generators.vscode.exclude', () => {
      ...
      expect(workspace.folders).toContainEqual({ path: 'lib1', name: 'lib1' })
      expect(workspace.folders).not.toContainEqual(expect.objectContaining({ path: 'lib2' }))
    })
