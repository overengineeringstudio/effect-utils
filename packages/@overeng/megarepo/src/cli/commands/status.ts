...
> =>
  Effect.gen(function* () {
    ...
    for (const [memberName, sourceString] of Object.entries(config.members)) {
      ...
      const sourceRefOpt = source !== undefined ? getSourceRef(source) : Option.none<string>();
      ...
      const duplicateWorktree =
        ...
        Option.isSome(sourceRefOpt) === true
          ? ((yield* detectDuplicateBranchWorktree({
              source,
              ref: sourceRefOpt.value,
            })) ?? undefined)
          : undefined;
    ...
    Effect.gen(function* () {
      ...
      const collectMemberSyncReasons = ({
        ...
        for (const member of memberList) {
          ...
          if (member.duplicateWorktree !== undefined) {
            workspaceSyncReasons.push(
              `Member '${memberLabel}' has duplicate canonical/legacy branch worktrees for '${member.duplicateWorktree.ref}'`,
            );
          }
