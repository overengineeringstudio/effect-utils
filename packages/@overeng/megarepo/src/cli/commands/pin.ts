  ...
  ({ member, checkout, dryRun, output }) =>
    Effect.flatMap(resolveOutputOption(output), (outputMode) =>
      run(
        ...
        (tui) =>
          Effect.gen(function* () {
            ...
            if (Option.isSome(checkout) === true) {
              ...
              // Git's branch registration must agree with the canonical store path.
              ...
              if (dryRun === true) {
                ...
                if (currentLockRef !== newRef)
                  lockChanges.push(`ref: ${currentLockRef} → ${newRef}`)
        ...
        Effect.provide(Layer.merge(outputModeLayer(outputMode), StoreLayer)),
