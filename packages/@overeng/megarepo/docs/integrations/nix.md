...
Megarepo solves this with **Nix Lock Sync**: during `mr lock sync`, `mr lock update`, and `mr lock apply`, it updates `flake.lock` and `devenv.lock` files in member repos to match the commits in `megarepo.lock`.
