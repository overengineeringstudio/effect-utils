...
import {
  workspaceBuckBinding,
  workspaceName,
  workspacePackages,
} from './buck2/workspace-packages.ts'
...
const projectReferenceNames = (): readonly string[] => {
  ...
  return [...seen].toSorted((left, right) => compareStrings({ left, right }))
}
...
const checkWorkspaceSources = projectReferenceNames()
  .map((name) => workspaceBuckBinding({ name, role: 'project' }).label)
  .toSorted((left, right) => compareStrings({ left, right }))
const runtimeWorkspaceSources = runtimeWorkspacePackageNames()
  .map((name) => workspaceBuckBinding({ name, role: 'production' }).label)
  .toSorted((left, right) => compareStrings({ left, right }))
const prefixForLabel = (label: string): string => {
  const name = Object.keys(workspacePackages).find(
    (candidate) =>
      workspaceBuckBinding({ name: candidate, role: 'production' }).label === label ||
      workspaceBuckBinding({ name: candidate, role: 'project' }).label === label,
  )
  if (name === undefined) throw new Error(`Unknown workspace Buck label: ${label}`)
  return workspaceBuckBinding({ name, role: 'production' }).packagePath
}
