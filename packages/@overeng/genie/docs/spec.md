...
## Generator Phase and Install Ordering

Satisfies R31 (generator phase) and R32 (install-arbitrated ordering); realizes decision 0004.

**Phase declaration (static, no import).** Each generator has a phase — `bootstrap` or `design-time` — declared by a `// @genie-phase bootstrap` line comment in the `.genie.ts` source. `design-time` is the default (unmarked). The phase is read from the raw source text (`parseGeneratorPhase`, `src/core/phase.ts`) with no import and no TypeScript parse: importing a `design-time` generator would itself require the runtime graph (e.g. `effect`), which is exactly what is unavailable before install. A per-file pragma (rather than a filename rule like "every `package.json.genie.ts` is bootstrap") is deliberate: R32 forbids a hardcoded catalog of install-input filenames, so the mark is an explicit per-generator declaration, not an inferred convention. In effect-utils the bootstrap set is the 35 `package.json.genie.ts` sources plus `pnpm-workspace.yaml.genie.ts` (36 generators); everything else — including weaver — is `design-time`.

**Phase-selected running.** `genie --phase <bootstrap|design-time>` restricts a `generate`/`check`/`dry-run` to generators of that phase (discovery reads the pragma of each discovered source and filters; absent the flag, all phases run). Duplicate-target rejection still runs over the full discovered set before filtering. The normal post-install `genie:run` runs all phases.

**Install-arbitrated ordering.** A devenv task `genie:bootstrap` runs `genie --phase bootstrap` to regenerate the bootstrap-phase install inputs, and `pnpm:install` depends on `genie:bootstrap`, so install consumes freshly-derived inputs. This makes the pre-install regeneration the _real_ ordering rather than a static proxy: a `bootstrap` generator that reaches a runtime-only package fails when run pre-install (and is caught early by `bootstrap-closure:check`), and — in principle — a generator whose output install needs but that is left unmarked does not run in `genie:bootstrap`, so install would see a stale/missing input.

**Enforcement reality in effect-utils (two structural gaps, stated precisely).** Decision 0004's "install is the arbiter via real ordering" holds only partially here, because of two repository-specific facts:

- **Source-mode genie cannot run cold.** The `genie` on `PATH` is the source-mode CLI (`bun bin/genie.tsx`), whose build layer resolves `effect`/`@effect/platform-node`/`@overeng/*` from `node_modules`. It therefore cannot run before the first install. `genie:bootstrap` is guarded to no-op when `node_modules` is absent (a fresh clone), so it never deadlocks the bootstrap; the pre-install regeneration only actually runs in the warm/steady-state case. The `bootstrap-closure:check` static gate — which reads pragmas and walks closures without importing — is what carries the "safe in a pre-install-shaped env" property, but note it itself imports `typescript` and so is fast local feedback rather than a truly `node_modules`-free proof.
- **Committed outputs blunt completeness.** Generated outputs are checked in (T01), so install always finds a `package.json`/`pnpm-workspace.yaml` on disk regardless of whether its generator is marked. A _forgotten_ bootstrap mark therefore does **not** break `pnpm:install` — install sees the committed (possibly stale) file and succeeds. What catches a stale committed output is the post-install `genie:check` drift gate (all phases) — but only where the cold-guard has skipped `genie:bootstrap`. In **CI** (`node_modules` is not cached — only the pnpm store and `.pnpm-home` are — so it is absent pre-install) `genie:bootstrap` skips, and `genie:check` sees and fails on the stale committed output. In a **warm local** tree (`node_modules` persists), `genie:bootstrap` regenerates the bootstrap outputs before `genie:check` runs, so local `check:all` **masks** stale committed bootstrap-output drift; CI is the gate that catches it. A wrong phase mark on a generator whose committed output is currently correct is latent until that source next changes. Truly install-arbitrated completeness would require a fresh-clone (no committed outputs) run — the P2 spot-check decision 0004 considered and did not adopt.

**Install-path blast radius.** `pnpm:install` is the most-depended-on task in the graph, and it now depends on `genie:bootstrap`, which runs a full `genie --phase bootstrap` generate + validation over the 35 `package.json` generators (including strict tsgo export-environment proofs where configured). This adds cost to every warm install and a new failure mode: a genie generation/validation/proof flake now fails `pnpm:install` and cascades to everything downstream. The cold-guard removes this from the fresh-clone path (it skips when `node_modules` is absent), so the cost/failure surface is the warm/steady-state case.

The net contract: `bootstrap-closure:check` enforces the _safety_ of the marked set (zero-tolerance), and the post-install `genie:check` drift gate enforces _output freshness_ across all phases (in CI/cold; masked warm for the bootstrap subset); the pre-install ordering is real and demonstrable warm but is not, in a committed-output source-mode repo, the sole structural guarantor 0004's abstract model assumes.
...
`genie:prepare` (the shared prerequisite hook) is distinct from `genie:bootstrap` (the pre-install bootstrap-phase generation task, see _Generator Phase and Install Ordering_): the former is a read-only prerequisite that `genie:run`/`genie:check`/`genie:watch` depend on; the latter writes the bootstrap-phase install inputs and is depended on by `pnpm:install`. They are kept separate because (a) `genie:bootstrap` is repository-local and does not mutate the exported shared `genie.nix` module, and (b) it does not add a direct writing edge to `genie:run`/`genie:watch`. It does **not** keep the writer out of `genie:check`'s dependency chain — `genie:check` depends on `pnpm:install`, which depends on `genie:bootstrap`, so the writer is transitively upstream of `genie:check` regardless. Masking is instead prevented by the cold-guard (below), not by task separation.
