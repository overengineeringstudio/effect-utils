...
**Scope: bootstrap-phase generators only (zero-tolerance).** The gate is scoped to generators declared `bootstrap`-phase (see _Generator Phase and Empirical Cold-Proof_ below); `design-time` generators are out of scope **by declaration**, not by allowlist. It fails on ANY bootstrap-phase violation — there is no baseline and no ratchet. The residual weaver generators (`genie/weaver-registry/*.genie.ts`) reach `effect` because the OTel semantic-convention contracts are Effect-Schema by design (`@overeng/otel-contract/src/registry.ts` imports `Schema` from `effect`); they are `design-time` (unmarked) and therefore structurally excluded rather than baselined. The check runs as a devenv task (`bootstrap-closure:check`) wired into `check:all` (not `check:quick`) — an existing check-flow surface, not a new Genie CLI subcommand. The shared task invokes the effect-utils checker with the importing repo's root, so downstream megarepo members can enforce the same contract over their own bootstrap-phase `.genie.ts` without carrying repo-local wrapper scripts. The lower-level `checkBootstrapClosure` walker remains exported from `@overeng/genie/node` for specialized callers.
