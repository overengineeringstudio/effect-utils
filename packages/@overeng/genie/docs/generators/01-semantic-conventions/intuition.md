...
you author ──► TS registry DSL ──► genie projects ──► weaver gate (check/diff/live-check)
                    │                                        ▲
                    └── conformance check ◄── otel-contract ─┘  (runtime, unchanged)
```

The formal constraints are in [requirements.md](./requirements.md); the mechanism and
the derisked weaver-fidelity details are in [spec.md](./spec.md); the two big undecided
forks (how to guarantee the conformance sweep is complete, and whether to eventually
fold otel-contract into the registry) are in
