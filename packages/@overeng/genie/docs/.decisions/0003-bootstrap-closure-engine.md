...
- **Enforcement is baseline + ratchet**: the pre-existing violations are baselined so the gate is green today; only new violations fail, and stale baseline entries are flagged so it ratchets down. (Superseded by decision 0004: the gate is now scoped to bootstrap-phase generators and zero-tolerance, with no baseline; the empirical `bootstrap:cold-proof` is the authority.)
- **Integration is a reusable devenv task** (`bootstrap-closure:check`) wired into `check:all`, mirroring `weaver:check` — not a new genie CLI subcommand. The task invokes the effect-utils checker with the importing repo's root, so downstream repos reuse the same discovery, phase filtering, and diagnostics without carrying local wrapper scripts. The `checkBootstrapClosure` walker remains exported from `@overeng/genie/node` for specialized callers.
