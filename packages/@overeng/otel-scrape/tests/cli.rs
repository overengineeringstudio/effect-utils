...
fn scope_version_is_build_correlated_and_schema_url_present() {
    ...
    let binary_version = String::from_utf8(version_out.stdout)
        ...
        .to_owned();
    let collector = TestCollector::start(200);
    // A distinctive runtime NixStamp. An installed product build-info record
    // wins; a plain Cargo build honors this runtime fallback.
    let out = otel_scrape()
        .env(
            "CLI_BUILD_STAMP",
...
// Without a runtime stamp, scope.version must exactly match the identity
// reported by the binary. This is `0.0.0+dev` for a development executable and
// the packaged machineVersion when the adjacent build-info file is present.
#[test]
