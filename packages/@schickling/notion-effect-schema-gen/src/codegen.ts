...
export function generateApiCode(
  ...
  const { includeWrite, generatorVersion } = parseGenerateOptions(options)
  ...
  const lines: string[] = [
    ...
    `// URL: ${dbInfo.url}`,
    ``,
    `import { NotionDatabases, NotionPages, type TypedPage } from '@schickling/notion-effect-client'`,
