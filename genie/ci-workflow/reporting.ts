...
import {
  ...
  workflowReportStatePrefix,
  workflowReportStateSuffix,
...
export const workflowReportPublisherStep = (
  ...
  run: [
    ...
    [
      ...
      `const statePrefix = ${JSON.stringify(workflowReportStatePrefix)}`,
      `const stateSuffix = ${JSON.stringify(workflowReportStateSuffix)}`,
      'const extractState = (body, label) => {',
      '  const startIndex = body.indexOf(statePrefix)',
      '  if (startIndex === -1) return undefined',
      '  const stateStartIndex = startIndex + statePrefix.length',
      '  const endIndex = body.indexOf(stateSuffix, stateStartIndex)',
      '  if (endIndex === -1) throw new Error(`${label} is missing the managed state suffix marker`)',
      '  const state = JSON.parse(body.slice(stateStartIndex, endIndex))',
      '  if (typeof state !== "object" || state === null || Array.isArray(state)) throw new Error(`${label} managed state must be an object`)',
      '  if (state._tag !== "WorkflowReportManagedState") throw new Error(`${label} managed state _tag is invalid`)',
      '  if (state.schemaVersion !== 1) throw new Error(`${label} managed state schemaVersion is invalid`)',
      '  if (typeof state.stateId !== "string" || state.stateId.length === 0) throw new Error(`${label} managed state stateId is required`)',
      '  return state',
      '}',
      'const targetState = extractState(readFileSync(commentBodyPath, "utf8"), "workflow report comment body")',
      ...
      '    extractState(comment.body, `issue comment ${comment.id}`)?.stateId === targetState.stateId,',
      '  )',
