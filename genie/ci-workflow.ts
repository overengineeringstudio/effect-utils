...
export const netlifyStorybookCommentStep = (site: string) => ({
  ...
  run: [
    ...
    [
      ...
      '}',
      '',
      'const validatePackage = (value, path) => {',
      '  const record = expectObject(value, path)',
      ...
      '  const packages = expectArray(record.packages, `${path}.packages`).map((pkg, index) =>',
      '    validatePackage(pkg, `${path}.packages[${index}]`),',
      '  )',
      '  return {',
      '    commitSha: expectString(record.commitSha, `${path}.commitSha`),',
      '    modeLabel: expectString(record.modeLabel, `${path}.modeLabel`),',
      ...
      '}',
      '',
      'const storybookRoot = "packages/@overeng"',
      'const site = expectString(process.env.STORYBOOK_PREVIEW_SITE, "STORYBOOK_PREVIEW_SITE")',
      'const suffix = process.env.STORYBOOK_PREVIEW_SUFFIX ?? ""',
      'const deployedAtUtc = expectIsoUtc(process.env.STORYBOOK_PREVIEW_DEPLOYED_AT_UTC, "STORYBOOK_PREVIEW_DEPLOYED_AT_UTC")',
      '',
      'const currentPackages = existsSync(storybookRoot)',
      '  ? readdirSync(storybookRoot, { withFileTypes: true })',
      '      .filter((entry) => entry.isDirectory() && existsSync(`${storybookRoot}/${entry.name}/storybook-static`))',
      '      .map((entry) => ({',
      '        packageName: entry.name,',
      '        url: `https://${entry.name}${suffix}--${site}.netlify.app`,',
      '      }))',
      '  : []',
      '',
      'currentPackages.forEach((pkg, index) => validatePackage(pkg, `currentPackages[${index}]`))',
      '',
      ...
      '    if (!latest) return []',
      '    return [`| ${packageName} | ${latest.url} | ${formatBerlin(latest.deployedAtUtc)} |`]',
      '  }),',
      ']',
      '',
      'if (nextState.commits.length > 0) {',
      ...
      '      if (!pkg) continue',
      '      visibleLines.push(`| ${packageName} | ${pkg.url} |`)',
      '    }',
      "    visibleLines.push('')",
      '  }',
      "  visibleLines.push('</details>')",
