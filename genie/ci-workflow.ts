...
export const netlifyDeployStep = () => ({
  ...
  run: [
    ...
    'tmp_metadata="/tmp/netlify-storybook-deploy.ndjson"',
    'rm -f "$tmp_log"',
    'rm -f "$tmp_metadata"',
    'export STORYBOOK_PREVIEW_METADATA_PATH="$tmp_metadata"',
    'if [ -z "${NETLIFY_AUTH_TOKEN:-}" ]; then',
    '  echo "::notice::Skipping Netlify deploy (NETLIFY_AUTH_TOKEN not available)"',
    '  exit 0',
    'fi',
    'if [ "${{ github.event_name }}" = "push" ] && [ "${{ github.ref }}" = "refs/heads/main" ]; then',
    `  ${runDevenvTasksBefore('netlify:deploy', '--input', 'type=prod')} 2>&1 | tee "$tmp_log"`,
    'elif [ "${{ github.event_name }}" = "pull_request" ]; then',
    `  ${runDevenvTasksBefore('netlify:deploy', '--input', 'type=pr', '--input', 'pr=${{ github.event.pull_request.number }}')} 2>&1 | tee "$tmp_log"`,
    'fi',
    'deploy_exit=${PIPESTATUS[0]}',
    'if [ "$deploy_exit" -ne 0 ]; then exit "$deploy_exit"; fi',
  ].join('\n'),
...
export const netlifyStorybookCommentStep = (site: string) => ({
  ...
  env: {
    GH_TOKEN: '${{ github.token }}',
    GH_REPO: '${{ github.repository }}',
    STORYBOOK_PREVIEW_METADATA_PATH: '/tmp/netlify-storybook-deploy.ndjson',
    STORYBOOK_PREVIEW_LOG_PATH: '/tmp/netlify-storybook-deploy.log',
  },
  run: [
    `site="${site}"`,
    deployModeScript,
    ...
    [
      ...
      '',
      'const metadataPath = expectString(process.env.STORYBOOK_PREVIEW_METADATA_PATH, "STORYBOOK_PREVIEW_METADATA_PATH")',
      'const deployedAtUtc = expectIsoUtc(process.env.STORYBOOK_PREVIEW_DEPLOYED_AT_UTC, "STORYBOOK_PREVIEW_DEPLOYED_AT_UTC")',
      '',
      'const currentPackages = existsSync(metadataPath)',
      '  ? readFileSync(metadataPath, "utf8")',
      '      .split("\\n")',
      '      .flatMap((line) => (line.trim().length === 0 ? [] : [validatePackageV3(JSON.parse(line), "currentPackages")]))',
      '  : []',
      '',
