...
export const netlifyStorybookCommentStep = (site: string) => ({
  ...
  run: [
    ...
    [
      ...
      '  if (record._tag !== stateTag) fail(`state._tag must be ${stateTag}`)',
      '  if (record.schemaVersion !== 1 && record.schemaVersion !== schemaVersion) fail(`state.schemaVersion must be 1 or ${schemaVersion}`)',
      '  if (record.timeZone !== timeZone) fail(`state.timeZone must be ${timeZone}`)',
      ...
      '',
      'const logPath = expectString(process.env.STORYBOOK_PREVIEW_LOG_PATH, "STORYBOOK_PREVIEW_LOG_PATH")',
      'const deployedAtUtc = expectIsoUtc(process.env.STORYBOOK_PREVIEW_DEPLOYED_AT_UTC, "STORYBOOK_PREVIEW_DEPLOYED_AT_UTC")',
      '',
      'const currentPackages = existsSync(logPath)',
      ...
      '        return [validatePackageV2(JSON.parse(line.slice(prefixIndex + prefix.length)), "currentPackages")]',
      '      })',
      '  : []',
      '',
      'currentPackages.forEach((pkg, index) => validatePackageV2(pkg, `currentPackages[${index}]`))',
      '',
      ...
      '      latestByPackage.set(pkg.packageName, { ...pkg, deployedAtUtc: commit.deployedAtUtc })',
      '    }',
      '  }',
      '}',
      '',
